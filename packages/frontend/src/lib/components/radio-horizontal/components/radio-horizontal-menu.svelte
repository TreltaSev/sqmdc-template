<script lang="ts">
	...
	import { setRadioHorizontalCtx } from '../ctx.svelte';
    ...
    let radiohorizontalCls = $state(cn(radiohorizontalClass, className));
    $effect(() => {
        radiohorizontalCls = cn(radiohorizontalClass, className)
    })

    // Set Context Here
    const {_class} = setRadioHorizontalCtx()
    _class.buttonClass = buttonClass
    _class.classButton = classButton

</script>

<div class={ radiohorizontalCls } {...rest}>
    {@render children?.()}
</div>
...
<!--@component
    ...
    These props are bindable and reactive. They are injected via `setRadioHorizontalCtx()` and accessed
